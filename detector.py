...
async def safe_download_media(app: Client, *args: typing.Any, **kwargs: typing.Any) -> BytesIO | None:
    """
    Safely downloads media, handling FloodWait exceptions by waiting and retrying.
    """
    while True:
        try:
            return await app.download_media(*args, **kwargs)  # type: ignore
...
async def bot_send_request(
    ...
    for bot_token in BOT_TOKENS_CYCLE:
        ...
        elif method == "editMessageText" and isinstance(response.get("description"),
                                                        str) and "message is not modified" in response["description"]:
            return
...
async def process_new_gift(app: Client, star_gift: StarGiftData) -> None:
    ...
    if not binary: return  # Could not download after retries
...
async def process_update_gifts(update_gifts_queue: UPDATE_GIFTS_QUEUE_T) -> None:
    while True:
        ...
        for new_star_gift in [
            ...
            logger.debug(f"Star gift updated with {new_star_gift.available_amount} available amount",
                         extra={"star_gift_id": str(new_star_gift.id)})
...
async def star_gifts_data_saver(star_gifts: StarGiftData | list[StarGiftData]) -> None:
    ...
    async with star_gifts_data_saver_lock:
        ...
        STAR_GIFTS_DATA.star_gifts = tuple(updated_gifts_list)
    ...
    while True:
        for star_gift_id, star_gift in {
            ...
            if await check_is_star_gift_upgradable(
                ...
                if not binary: continue  # Could not download after retries
